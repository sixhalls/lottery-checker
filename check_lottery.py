...
PRESETS = {
    ...
    "ssq": [
        # Example: {"front": [1,5,12,22,28,33], "back": [8]},
        # Add your SSQ numbers here
    ]
}
...
def update_html(dlt_data, ssq_data):
    ...
    if dlt_data:
        ...
        html = re.sub(r'dlt: \{ period: [^}]+\}', dlt_json, html)
    ...
    if ssq_data:
        ...
        html = re.sub(r'ssq: \{ period: [^}]+\}', ssq_json, html)
    ...
    for game, presets in PRESETS.items():
        ...
        for i, preset in enumerate(presets):
            ...
            if results:
                ...
                push_msgs.append(f"🎉 {game_name}第{period}期 第{i+1}组：<b>{best['name']}</b><br>中奖汇总：{summary}<br>开奖：{draw_front} + {draw_back}<br>你的：{u_front} + {u_back}")
            else:
                f_hits = len([n for n in u_front if n in draw_front])
                b_hits = len([n for n in u_back if n in draw_back])
                if f_hits > 0 or b_hits > 0:
                    push_msgs.append(f"{game_name}第{period}期 第{i+1}组：未中奖（{game_name=='大乐透' and '前区' or '红球'}命中{f_hits}，{'后区' if game=='dlt' else '蓝球'}命中{b_hits}）")
